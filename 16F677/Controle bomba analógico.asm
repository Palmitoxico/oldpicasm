...
processor 16F677
include <p16f677.inc>
__config _WDT_OFF & _PWRTE_ON & _XT_OSC & _BOR_ON & _CP_OFF & _MCLRE_ON
...
#define Multiplicador_FLX	.45						;Define a fração para ajustar o fluxo
#define Divisor_FLX			.540					;
#define OffSet_FLX			0xF466					;Valor do off set do cáculo para fluxo = -2970
...
Main
	...
	MOVWF	ADCON1 ;
	BANKSEL	TRISB ;
	BSF		TRISB,5 ;Set RB5 to input
	BANKSEL	ANSEL ;
	BSF		ANSEL,2 ;Set RB5 to analog
	BANKSEL	ADCON0 ;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;Rotina de comparação de 16 bits
	movf	Media_Fluxo_H,w						;Verifica se a leitura foi abaixo de 0,9 volt (valor 184)
	sublw	high(.184)
Are_they_equal:
	; Are they equal ?
	skpz
	...
	sublw	low(.184)	; subtract Y-X
results16:
	btfss	STATUS,C
	goto	Calcular_Fluxo
												;Caso seja, zera o fluxo
	movlw	Low(Inicial_01)
	movwf	OffSet_Str_L
	movlw	High(Inicial_01)
	movwf	OffSet_Str_H
...
Calcular_Fluxo:
	...
	movff	Multiplicando_L, Media_Fluxo_L
	movff	Multiplicando_H, Media_Fluxo_H
	movlf	Multiplicador_L, low(Multiplicador_FLX)
	movlf	Multiplicador_H, high(Multiplicador_FLX)
	call	MULV16
	movff	Media_Fluxo_L, Resultado_H
	movff	Media_Fluxo_H, Resultado_H
	
	movlw	low(OffSet_FLX)
	addwf	Resultado_L,F
	btfsc	STATUS,C
	incf	Resultado_H
	movlw	high(OffSet_FLX)
	addwf	Resultado_H,F
	
	movff	Dividendo_L, Resultado_L
	movff	Dividendo_H, Resultado_H
	
	movlf	Divisor_L, low(Divisor_FLX)
	movlf	Divisor_H, high(Divisor_FLX)	
	call	DIVV16
	
	movff	VAR_L, Resultado_L
	movff	VAR_H, Resultado_H
